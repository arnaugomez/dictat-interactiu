...
export class DictatService extends Context.Service<DictatService, {
...
export const DictatServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getById = Effect.fn("Dictat.getById")(function* (id: string, userId: string) {
      const row = db.query.dictats.findFirst({
    ...
    const create = Effect.fn("Dictat.create")(
      function* (
        userId: string,
        data: {
          text: string
    ...
    const update = Effect.fn("Dictat.update")(
      function* (
        id: string,
        userId: string,
        data: {
        ...
        const existing = db.query.dictats.findFirst({
          where: and(eq(schema.dictats.id, id), eq(schema.dictats.userId, userId)),
        }).sync()
        if (!existing) {
          return yield* new NotFoundError({ message: "Dictat not found" })
        ...
        const updated = db.query.dictats.findFirst({
