...
Use available exploration tools to navigate the codebase naturally. Do NOT follow rigid heuristics — explore organically and note where you experience friction:
...
- **Dependency category**: See the Dependency Categories reference below for the four categories
...
If the user explicitly authorized subagents, spawn 3+ subagents in parallel. Each must produce a **radically different** interface for the deepened module. If subagents are not available or not authorized, produce the alternative designs yourself and clearly label them as independent design options.
...
4. Dependency strategy (how deps are handled — see Dependency Categories reference below)
5. Trade-offs
...
Create a refactor RFC as a GitHub issue using `gh issue create`. Use the issue template below. Do NOT ask the user to review before creating — just create it and share the URL. If GitHub tooling is unavailable, provide the complete issue title and body instead.
